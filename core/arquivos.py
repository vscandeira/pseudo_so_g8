...
class GerenciadorArquivos:
    ...
    def iniciar_filesystem(self, operacoes_arquivos):
        ...
        for oc in operacoes_arquivos[1]:
            for i in range(oc[2]):
                self.mapa_ocupacao[oc[1]+i] = oc[0]  # Preenche o mapa de ocupação com os blocos já alocados
        i = 1
        ...
        for i in range(len(self.operacoes)):
            if (self.operacoes[i].id_operacao == id_op) and (not self.operacoes[i].executado):
                ...
                if self.operacoes[i].cod_operacao == 0:
                    self.criar_arquivo(i)
                elif self.operacoes[i].cod_operacao == 1:
                    self.deletar_arquivo(i)
                else:
            ...
            if self.mapa_ocupacao[i] == self.livre:
                ...
                if num_livres >= self.operacoes[index].num_blocos:
                    ...
                    while (escrito < self.operacoes[index].num_blocos):
                        self.mapa_ocupacao[i + escrito] = self.operacoes[index].nome_arquivo
                        escrito += 1
    ...
    def deletar_arquivo(self, index):
        for i in range(self.num_blocos):
            if(self.mapa_ocupacao[i] == self.operacoes[index].nome_arquivo):
                j = i
                while (self.mapa_ocupacao[j] == self.operacoes[index].nome_arquivo):
...
class OperacaoArquivo:
    def __init__(self, pid, cod_operacao, nome_arquivo, id_operacao, num_blocos=None):
       self.pid = pid
